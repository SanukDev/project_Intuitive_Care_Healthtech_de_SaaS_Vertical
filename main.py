...
df_relatorio['CEP'] = df_relatorio['CEP'].astype(str)
df_relatorio['DDD'] = df_relatorio['DDD'].astype(str)
df_relatorio['Telefone'] = df_relatorio['Telefone'].astype(str)
df_relatorio['Fax'] = df_relatorio['Fax'].astype(str)
...
print(f'\nThere are NaN values? \n')
print(df_consolidado.isna().sum())

print(df_consolidado['REG_ANS'].duplicated().sum())
# ------------------------------- merger the data

# df_final = df_consolidado.merge(df_relatorio, right_on=['REGISTRO_OPERADORA'], left_on=['REG_ANS'], how='right')
df_final = df_relatorio.merge(df_consolidado, left_on='REGISTRO_OPERADORA', right_on='REG_ANS', how='left')
...
df_despesas['CNPJ'] = df_final['CNPJ'].astype(str)
...
df_despesas['RegistroANS'] = df_final['REG_ANS'].astype('int64')
...
df_despesas = df_despesas.reset_index(drop=True)
...
#df_relatorio
df_relatorio = df_relatorio.fillna('NULL')
